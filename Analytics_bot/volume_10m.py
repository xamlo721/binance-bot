...
class FileHandler(FileSystemEventHandler):
    ...
    def on_created(self, event):
        if not event.is_directory and event.src_path.endswith('.csv'):
            ...
            logger.info(vol_10m_SCRIPT_NAME + f"Обнаружен новый файл: {event.src_path}")
    ...
    def wait_for_file_stabilization(self, filepath, check_interval=0.5, max_checks=10):
        ...
        logger.warning(vol_10m_SCRIPT_NAME + f"Файл {filepath} не стабилизировался за {max_checks * check_interval} секунд")
        return False
    ...
    def calculate_10m_volume(self, files):
        ...
        for file in files:
            try:
                ...
                logger.error(vol_10m_SCRIPT_NAME + f"Ошибка чтения файла {file}: {e}")
                continue
    ...
    def process_file(self, filepath):
        ...
        if len(latest_files) < 10:
            ...
            logger.warning(vol_10m_SCRIPT_NAME + f"Найдено только {len(latest_files)} файлов, нужно 10. Пропускаем.")
            return
        ...
        if volume_df.empty:
            ...
            logger.info(vol_10m_SCRIPT_NAME + "Нет данных для агрегации")
            return
        ...
        logger.info(vol_10m_SCRIPT_NAME + f"Создан файл: {output_path}")
        #logger.info(f"Обработано {len(volume_df)} тикеров")
        logger.info(vol_10m_SCRIPT_NAME + f"Обработано {len(volume_df)} тикеров")
...
def cleanup_result_files():
    ...
    while len(files) >= vol_10m_MAX_RESULT_FILES + 1:
        ...
        try:
            ...
            logger.info(vol_10m_SCRIPT_NAME + f"Удален старый файл: {os.path.basename(oldest_file)}")
        except OSError as e:
            logger.error(vol_10m_SCRIPT_NAME + f"Ошибка удаления файла {oldest_file}: {e}")
...
def main():
    ...
    try:
        ...
        logger.info(vol_10m_SCRIPT_NAME + "Мониторинг остановлен")
